...
/*	from OpenSolaris "presvr4.c	1.12	06/02/27 SMI"	*/

/*
 * Portions Copyright (c) 2007 Gunnar Ritter, Freiburg i. Br., Germany
 *
 * Sccsid @(#)presvr4.c	1.4 (gritter) 2/25/07
 */
...
presvr4(char *pkg, int a_nointeract)
{
	...
	(void) snprintf(path, sizeof (path), "%s/install/UNINSTALL",
			"/install");
	tmpcmd = malloc(strlen(tmpdir) + 17);
	sprintf(tmpcmd, "%s/UNINSTALLXXXXXX", tmpdir);
	if (close(mkstemp(tmpcmd)) < 0 ||
			copyf(path, tmpcmd, 0) || chmod(tmpcmd, 0500)) {
