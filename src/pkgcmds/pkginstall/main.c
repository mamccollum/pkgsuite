...
/*	from OpenSolaris "main.c	1.113	07/02/05 SMI"	*/

/*
 * Portions Copyright (c) 2007 Gunnar Ritter, Freiburg i. Br., Germany
 *
 * Sccsid @(#)main.c	1.8 (gritter) 2/26/07
 */
...
static void	ck_w_dryrun(int (*func)(void), int type);
...
#define	DEFPATH		SBINDIR ":" BINDIR ":" "/sbin:/usr/sbin:/usr/bin"
...
merg_pkginfos(struct cl_attr **pclass, struct cl_attr ***mpclass)
{
	...
	for (param[0] = '\0'; (oldValue = fpkgparam(fp, param)) != NULL;
		...
		/*
		 * if there is no new value, and a zone attribute is being
		 * changed, it is the same as setting the zone package
		 * attribute to 'false' - make sure current setting is 'false'.
		 */

		if ((newValue == NULL) && (setZoneAttribute == B_TRUE) &&
		...
		/* old and new values different: error if zone parameter */
	...
	/*
	 * verify that no zone attribute has been set to an invalid value
	 */
	...
	if ((newValue != NULL) && (*SUNW_PKG_ALLZONES == '\0') &&
		...
		/* change ALLZONES from "true" to "false" (unset) */
			progerr(ERR_MERGINFOS_SET_ZONEATTR, pkgName,
				pkgVersion, PKG_ALLZONES_VARIABLE,
				newValue);
			return (1);
	}
	...
	if ((newValue != NULL) && (*SUNW_PKG_THISZONE == '\0') &&
		...
		/* change THISZONE from "true" to "false" (unset) */
			progerr(ERR_MERGINFOS_SET_ZONEATTR, pkgName,
				pkgVersion, PKG_THISZONE_VARIABLE,
				newValue);
			return (1);
	}
	...
	if ((newValue != NULL) && (*SUNW_PKG_HOLLOW == '\0') &&
		...
		/* change HOLLOW from "true" to 'false" (unset) */
			progerr(ERR_MERGINFOS_SET_ZONEATTR, pkgName,
				pkgVersion, PKG_HOLLOW_VARIABLE,
				newValue);
			return (1);
	}
 ...
 */
static void
ck_w_dryrun(int (*func)(void), int type)
{
	int n;
