...
/*	from OpenSolaris "main.c	1.50	06/11/17 SMI"	*/
...
/* splpkgmap.c */
extern int	splpkgmap(struct cfent **eptlist, int eptnum, char *order[],
    fsblkcnt_t bsize, fsblkcnt_t frsize, fsblkcnt_t *plimit,
    fsblkcnt64_t *pilimit, fsblkcnt_t *pllimit);
/* scriptvfy.c */
...
#define	TEXT_DOMAIN "SYS_TEST"
	 ...
	 */
	t_pkgmap = malloc(strlen(tmpdir) + 14);
	sprintf(t_pkgmap, "%s/tmpmapXXXXXX", tmpdir);
	if (close(mkstemp(t_pkgmap)) < 0) {
	...
	t_pkginfo = malloc(strlen(tmpdir) + 15);
	sprintf(t_pkginfo, "%s/pkginfoXXXXXX", tmpdir);
	close(mkstemp(t_pkginfo));
