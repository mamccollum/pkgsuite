...
/*	from OpenSolaris "splpkgmap.c	1.26	06/11/17 SMI"	*/

/*
 * Portions Copyright (c) 2007 Gunnar Ritter, Freiburg i. Br., Germany
 *
 * Sccsid @(#)splpkgmap.c	1.3 (gritter) 2/25/07
 */
...
static long	bpkginfo; 	/* blocks used by pkginfo file */
static char	**dirlist;
static short	volno; 		/* current part */
static int	nparts = -1; 	/* total number of parts */
static int	nclass;
static ulong_t 	DIRSIZE;
static struct	class_type *cl;

static int	nodecount(char *path);
static int	store(struct data **, int, char *, long, fsblkcnt64_t);
static void	addclass(char *aclass, int vol);
static void	allocnode(char *path);
static void	newvolume(struct data **, int, long limit, fsblkcnt64_t);
static void	sortsize(struct data *f, struct data **sf, int eptnum);

int
splpkgmap(struct cfent **eptlist, int eptnum, char *order[], fsblkcnt_t bsize,
    fsblkcnt_t frsize, fsblkcnt_t *plimit, fsblkcnt64_t *pilimit,
    fsblkcnt_t *pllimit)
{
	struct data	*f, **sf;
	struct cfent	*ept;
...
static int
store(struct data **sf, int eptnum, char *aclass, long limit,
    fsblkcnt64_t ilimit)
{
	int	i, svnodes = 0, choice, select;
	long	btemp, ftemp;
...
static void
newvolume(struct data **sf, int eptnum, long limit, fsblkcnt64_t ilimit)
{
	register int i;
	int	newnodes;
