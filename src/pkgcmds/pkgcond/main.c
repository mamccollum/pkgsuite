...
/*	from OpenSolaris "main.c	1.6	06/11/17 SMI"	*/
...
static void		checkForSparseRootMount(GLOBALDATA_T *a_gdt,
				char *a_mountPoint, char *a_fsType,
				char *a_mountOptions);
...
static	int	recursion = 0;
	...
	/* $ROOTDIR/etc/zones must not exist */

	r = testPath(TEST_NOT_EXISTS, "%s/%s", rootPath, "/etc/zones");
	if (r != R_SUCCESS) {
		/* $R/etc/zones cannot exist in non-global zone */
	...
	if ((a_gdt->gd_parentZoneName != (char *)NULL) &&
		(a_gdt->gd_currentZoneName != (char *)NULL) &&
	...
	if ((a_gdt->gd_parentZoneType != (char *)NULL) &&
	...
	if ((a_gdt->gd_currentZoneType != (char *)NULL) &&
		(strcmp(a_gdt->gd_currentZoneType, "global") == 0)) {
...
static	int	recursion = 0;
	...
	for (nn = 0, n = 0; n < listSize; n++) {
		long	mplen = strlen(list[n].fsi_mountPoint);
		if (rootPathLen < mplen) {
			/* root path is longer than target, ignore */
			continue;
		}
		if (strncmp(rootPath, list[n].fsi_mountPoint, mplen) == 0) {
	...
	log_msg(LOG_MSG_DEBUG, DBG_PWRT_INFO,
		rootPath, list[nn].fsi_mountPoint, list[nn].fsi_fsType,
 ...
 *		a_mountPoint - mount point to insert (is sort key)
...
static void
sortedInsert(FSI_T **r_list, long *a_listSize, char *a_mountPoint,
	char *a_fsType, char *a_mntOptions)
{
	...
	assert(a_mountPoint != (char *)NULL);
	assert(a_fsType != (char *)NULL);
	assert(a_mntOptions != (char *)NULL);

	/* entry debugging info */

	echoDebug(DBG_SINS_ENTRY, a_mountPoint, a_fsType, a_mntOptions);
	...
	if (listSize == 0) {
		...
		list[0].fsi_mountPoint = strdup(a_mountPoint);
		list[0].fsi_fsType = strdup(a_fsType);
		list[0].fsi_mntOptions = strdup(a_mntOptions);

		/* second entry is all NULL - end of entry marker */
		list[1].fsi_mountPoint = (char *)NULL;
		list[1].fsi_fsType = (char *)NULL;
		list[1].fsi_mntOptions = (char *)NULL;
	...
	for (n = 0; n < listSize; n++) {
		...
		c = strcmp(list[n].fsi_mountPoint, a_mountPoint);
		...
		if (c == 0) {
			...
			echoDebug(DBG_SORTEDINS_SKIPPED,
				n, list[n].fsi_mountPoint, a_fsType,
		...
		list[n].fsi_mountPoint = strdup(a_mountPoint);
	...
	list[listSize-1].fsi_mountPoint = strdup(a_mountPoint);
...
#ifndef	__sun
static int	_getmntent(FILE *, struct mnttab *);
#define	getmntent(a, b)	_getmntent(a, b)
#endif
static int
calculateFileSystemConfig(GLOBALDATA_T *a_gdt)
{
	...
	list[0].fsi_mountPoint = (char *)NULL;
	list[0].fsi_fsType = (char *)NULL;
	list[0].fsi_mntOptions = (char *)NULL;
	listSize = 0;
	...
	while (getmntent(fp, &mntbuf) == 0) {
		...
		checkForSparseRootMount(a_gdt, mntbuf.mnt_mountp,
			mntbuf.mnt_fstype, mntbuf.mnt_mntopts);
	}
 ...
 * Arguments:	a_gdt - global data structure to place sorted entries into
 *		a_mountPoint - pointer to string representing mount point
 *		a_fsType - pointer to string representing file system type
 *		a_mountOptions - pointer to string representing the options
...
static void
checkForSparseRootMount(GLOBALDATA_T *a_gdt, char *a_mountPoint,
	char *a_fsType, char *a_mountOptions)
{
	/* entry assertions */
	...
	if (mountOptionPresent(a_mountOptions, MNTOPT_RO) != R_SUCCESS) {
		return;
	}
	...
	/*
	 * file system is read-only lofs mount: return if not
	 * one of the defined "sparse root" file systems
	 */

	if ((strncmp(a_mountPoint, "/usr", 4) != 0) &&
		(strncmp(a_mountPoint, "/lib", 4) != 0) &&
		(strncmp(a_mountPoint, "/platform", 9) != 0) &&
		(strncmp(a_mountPoint, "/sbin", 5) != 0)) {
		return;
	}

	/* a defined sparse root file system is lofs read-only mounted */

	echoDebug(DBG_CKSR_FSREADONLY, a_mountPoint, a_fsType);
...
resolvePath(char **r_path)
{
	...
	if (realpath(*r_path, resolvedPath) != (char *)NULL) {
		free(*r_path);
