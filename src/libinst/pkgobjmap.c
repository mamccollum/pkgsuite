...
/*	from OpenSolaris "pkgobjmap.c	1.7	06/11/17 SMI"	*/

/*
 * Portions Copyright (c) 2007 Gunnar Ritter, Freiburg i. Br., Germany
 *
 * Sccsid @(#)pkgobjmap.c	1.3 (gritter) 2/25/07
 */
...
nblks(short fsys_entry, struct cfextra *ext)
{
	...
	fsblkcnt_t frag_size;

	block_size = get_blk_size_n(fsys_entry);
	frag_size = get_frag_size_n(fsys_entry);
...
cp_cfent(struct cfent *cf_ent, struct cfextra *el_ent)
{
	...
	/*
	 * If this is an object that can be copied from the medium, then
	 * ainfo.local will be set and the super-structure pointers will need
	 * to be copied over as well. These are also pointing to a malloc'd
	 * string.
	 */
	if (el_ent->cf_ent.ainfo.local != NULL) {
		tp->cf_ent.ainfo.local = el_ent->client_local;
		tp->server_local = el_ent->server_local;
		tp->client_local = el_ent->client_local;
	} else {
		tp->server_local = NULL;
		tp->client_local = NULL;
	}
