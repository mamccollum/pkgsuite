...
/*	from OpenSolaris "pkgops.c	1.13	06/11/17 SMI"	*/

/*
 * Portions Copyright (c) 2007 Gunnar Ritter, Freiburg i. Br., Germany
 *
 * Sccsid @(#)pkgops.c	1.5 (gritter) 2/26/07
 */
...
#define	PKGINFO_CMD	BINDIR "/pkginfo"

#define	GLOBALZONE_ONLY_PACKAGE_FILE_PATH	\
					VSADMDIR "/install/gz-only-packages"
...
pkgRemovePackageFromGzonlyList(char *a_rootPath, char *a_pkgInst)
{
	...
	(void) strftime(timeb, sizeof (timeb), timefmt, timep);
	...
	char a_mode, time_t a_modtime, char a_ftype, unsigned long a_cksum)
{
	...
	if (statbufSrc.st_mtime != statbufDst.st_mtime) {
		echoDebug(DBG_PKGOPS_MOD_MISMATCH,  a_src,
			statbufSrc.st_mtime, dstpath,
			statbufDst.st_mtime);
		return (B_FALSE);
	}
	...
	if (statbufDst.st_mtime != a_modtime) {
		echoDebug(DBG_PKGOPS_MOD_MISMATCH, dstpath,
			statbufDst.st_mtime, "source", a_modtime);
		return (B_FALSE);
	}
